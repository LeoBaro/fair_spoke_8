...
@torch.no_grad()
def hyperbolic_distance(x_space, x_time, y_space, y_time, curvature):
    x_space, y_space = x_space.float(), y_space.float()
    curvature = curvature.float()
    xy_inner = x_space @ y_space.T - x_time * y_time.T
    lorentzian_distance = torch.rsqrt(curvature) * torch.acosh(torch.clamp(-curvature * xy_inner, min=1e-8))
    return lorentzian_distance
...
@torch.no_grad()
def geodesic_motion(x_space, x_time, y_space, y_time,curvature,flow):

    x_space, y_space = x_space.float(), y_space.float()
    curvature = curvature.float()

    space_part = torch.cosh(torch.sqrt(curvature) * flow) * x_space + torch.sinh(torch.sqrt(curvature) * flow) * y_space
    time_part = torch.cosh(torch.sqrt(curvature) * flow) * x_time + torch.sinh(torch.sqrt(curvature) * flow) * y_time
