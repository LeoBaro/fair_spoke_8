...
from utils import create_config, create_output_folder, create_results_and_log_folders
    ...
    for num_workers in workers:
        for batch_size in batch_sizes:
            for execution in range(num_executions):
                current_output_folder, current_log_folder = create_results_and_log_folders(output_folder, suffix=f"_nw{num_workers}_bs{batch_size}_e{execution}")
                try:
                    took = run_pipeline(filtering_step_name, num_workers, batch_size, shards_path, current_output_folder)
                    with open(result_file, "a", encoding="utf-8") as f:
                        f.write(f"{num_workers},{batch_size},{execution},{took}\n")
...
def cli():
    ...
    parser.add_argument("--filtering_step_name", type=str, required=True)
...
def main(args):
    ...
    with open(result_file, "w", encoding="utf-8") as f:
        f.write("num_workers,batch_size,execution,took\n")

    cleanup()

    execution_loop(args.filtering_step_name, num_workers_list, batch_sizes, num_executions, shards_path, output_folder, result_file)
